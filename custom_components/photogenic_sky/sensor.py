...
MOON_PHASE_ILLUMINATION = {
    "new_moon": 0, "waxing_crescent": 15, "first_quarter": 50,
    "waxing_gibbous": 85, "full_moon": 100, "waning_gibbous": 85,
    "last_quarter": 50, "waning_crescent": 15,
}
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    location_name = location_data.get("location_name", config_entry.title)
...
class PhotogenicSkySensor(SensorEntity):
    ...
    async def async_update(self):
        ...
        # --- URL FIX: Parameters are now defined clearly and separately ---
        ...
        url = (
            ...
            f"&current={current_params}&daily={daily_params}&timezone=auto"
        )
        ...
        try:
            ...
            # Set a score of 0 and a summary so the sensor doesn't go blank
            self._photogenic_score = 0
        ...
        if not current or not daily:
            _LOGGER.error("API response missing 'current' or 'daily' data sections.")
            return
    ...
    def _calculate_main_score(self, sun_elevation, current):
        """Calculate the main photogenic score."""
        ...
        if sun_elevation < -6:
            lighting_condition, score, summary = "Night", 50, "Night time. See Astro card for sky details."
